...
@app.post("/startworking/{user_id}/{festival_id}")
def start_working(user_id: int, festival_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    db_user = crud.get_user(db, user_id)
    ...
    if db_festival is None:
        raise HTTPException(status_code=404, detail="Festival not found")
    db_user.festivals.append(db_festival)
    db.commit()

    return f"Gebruiker {db_user.email} is begonnen met werken op Festival {db_festival.naam}"